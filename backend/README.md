...
- `sequenceNo` позволяет упорядочивать сообщения и отбрасывать устаревшие,
  ...
  содержит виртуальное время модели. Пропуски номеров допустимы при
  объединении промежуточных состояний для медленного клиента.
...
только сессии со статусом `running`; `pause` замораживает виртуальное время,
`resume` продолжает расчёт, `complete` исключает сессию из цикла. Достижение
`maxVirtualTimeMs` автоматически завершает сессию.

- `KTK_SIMULATION_TICK_INTERVAL_MS` — интервал между расчётами в реальном
  времени.
- `KTK_SIMULATION_STEP_MS` — величина одного шага виртуального времени.
- `KTK_SIMULATION_AUTO_RUN` — включение автоматического цикла.

Значения по умолчанию `1000/1000` означают одну виртуальную секунду за одну
реальную. Соотношение времени является **учебным допущением**, а не
подтверждённым производственным нормативом.
